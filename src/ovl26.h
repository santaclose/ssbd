...
extern s32
	dMnBattlePanelColorIndexes[4]; // 0x8013B7B8[4]; // panel color indexes
...
extern s32
	dMnBattleTokenPickupDisplayOrders[4]; // 0x8013B840[4]; // display orders
										  // for cursors on token pickup
extern s32
	dMnBattleTokenPlaceHeldDisplayOrders[4]; // 0x8013B850[4]; // display
											 // orders for cursors holding
											 // tokens on token placement
extern s32 dMnBattleTokenPlaceUnheldDisplayOrders
	[4]; // 0x8013B860[4]; // display orders for cursors not holding tokens on
		 // token placement
...
extern Vec2f dMnBattleCursorStartingPositions[4]; // 0x8013B8AC[4]; // starting
												  // coords for cursors
extern s32 dMnBattleCursorStartingDisplayOrders
	[4]; // 0x8013B8CC[4]; // display orders for cursors on initial load
extern intptr_t dMnBattleTokenOffsetsNoCPU[4]; // 0x8013B8DC; // token offsets
											   // not including cpu
extern s32
	dMnBattleTokenStartingDisplayOrders[4]; // 0x8013B8EC; // display orders
...
extern f32
	dMnBattleWhiteCircleSizes[12];	// 0x8013B90C[12]; // white circle size
...
extern mnCharPanelBattle
	gMnBattlePanels[GMMATCH_PLAYERS_MAX]; // 0x8013BA88[GMMATCH_PLAYERS_MAX];
...
extern s32
	gMnBattleControllerOrderArray[4]; // 0x8013BD90; // -1 if no controller
...
// Offsets
extern intptr_t FILE_000_COLON_IMAGE_OFFSET
	= 0xDCF0; // file 0x000 image offset for colon

extern intptr_t FILE_011_TYPE_CP_IMAGE_OFFSET
	= 0xFF8; // file 0x011 image offset for CP type image
extern intptr_t FILE_011_HANDICAP_IMAGE_OFFSET
	= 0x1108; // file 0x011 image offset for Handicap image
extern intptr_t FILE_011_CPU_LEVEL_IMAGE_OFFSET
	= 0x1218; // file 0x011 image offset for CPU Level image
extern intptr_t FILE_011_START_IMAGE_OFFSET
	= 0x1378; // Press Start's "Start" texture
extern intptr_t FILE_011_PRESS_IMAGE_OFFSET
	= 0x14D8; // Press Start's "Press" texture
extern intptr_t FILE_011_INFINITY_IMAGE_OFFSET
	= 0x3EF0; // file 0x011 image offset for infinity symbol
extern intptr_t FILE_011_PICKER_TIME_IMAGE_OFFSET
	= 0x48B0; // file 0x011 image offset for Time picker texture
extern intptr_t FILE_011_PICKER_STOCK_IMAGE_OFFSET
	= 0x5270; // file 0x011 image offset for Stock picker texture
extern intptr_t FILE_011_CURSOR_POINTER_IMAGE_OFFSET
	= 0x76E8; // file 0x011 image offset for pointer cursor
extern intptr_t FILE_011_PANEL_DOOR_L_IMAGE_OFFSET = 0xCDB0;
extern intptr_t FILE_011_PANEL_DOOR_R_IMAGE_OFFSET = 0xDFA0;
extern intptr_t FILE_011_ARROW_L_IMAGE_OFFSET
	= 0xECE8; // file 0x011 image offset for left arrow
extern intptr_t FILE_011_ARROW_R_IMAGE_OFFSET
	= 0xEDC8; // file 0x011 image offset for right arrow
extern intptr_t FILE_011_READY_TO_FIGHT_IMAGE_OFFSET
	= 0xF448; // Ready to Fight banner text
extern intptr_t FILE_011_READY_TO_FIGHT_BG_IMAGE_OFFSET
	= 0xF530; // Ready to Fight banner bg
extern intptr_t FILE_011_PANEL_IMAGE_OFFSET = 0x104B0;
extern intptr_t FILE_011_BACK_IMAGE_OFFSET
	= 0x115C8; // file 0x011 image offset for
...
extern s32 FILE_013_PORTRAIT_FIRE_BG_IMAGE_OFFSET
	= 0x24D0; // file 0x013 image offset for portrait bg (fire) image

extern intptr_t FILE_015_BACKGROUND_IMAGE_OFFSET
	= 0x440; // file 0x015 image offset for background tile

extern intptr_t FILE_016_WHITE_CIRCLE_OFFSET_1
	= 0x408; // AObj? for white circle
extern intptr_t FILE_016_WHITE_CIRCLE_OFFSET_2
	= 0x568; // DObjDesc for white circle
...
GObj* func_8000B93C(u32, void*, s32, u32, void*, s32, s64, s32, s32, s32, s32,
					s32, s32);				   /* extern */
...
s32 mnBattleReorderCursorsOnPlacement(
	s32 port_id, s32 held_token_id); // doesn't actually return anything but
									 // required to match
