...
void mpObjectProc_SetLastWallCollideStats(f32 pos, s32 line_id, u32 flags,
										  Vec3f* angle);
void mpObjectProc_GetLastWallCollideStats(f32* pos, s32* line_id, u32* flags,
										  Vec3f* angle);
...
sb32 mpObjectProc_UpdateMapProcMain(mpCollData* coll_data,
...
sb32 mpObjectProc_CheckTestGroundCollisionAdjNew(mpCollData* coll_data,
												 sb32 (*proc_map)(GObj*),
												 GObj* gobj);
...
sb32 mpObjectProc_CheckSetFighterCliffEdge(GObj* fighter_gobj,
										   s32 ground_line_id);
sb32 mpObjectProc_CheckSetFighterEdge(GObj* fighter_gobj, s32 ground_line_id);
sb32 mpObjectProc_RunFighterAllCollisions(mpCollData* coll_data,
										  GObj* fighter_gobj, u32 flags);
...
sb32 ftMap_CheckGroundBreakEdgeProcMap(GObj* fighter_gobj,
									   void (*proc_map)(GObj*));
...
sb32 func_ovl2_800DDF74(GObj* fighter_gobj, ftStruct* fp,
						ftAttributes* attributes, DObj* target_joint,
						Vec3f* vec);
...
sb32 mpObjectProc_RunFighterSpecialCollisions(mpCollData* coll_data,
											  GObj* fighter_gobj, u32 flags);
sb32 ftMap_CheckAirLanding(GObj* fighter_gobj);
sb32 mpObjectProc_ProcFighterGroundProcMap(GObj* fighter_gobj,
										   void (*proc_map)(GObj*));
...
sb32 mpObjectProc_ProcFighterPassCliff(GObj* fighter_gobj,
									   sb32 (*proc_map)(GObj*));
sb32 mpObjectProc_ProcFighterCliff(GObj* fighter_gobj);
sb32 mpObjectProc_ProcFighterCliffProcMap(GObj* fighter_gobj,
										  void (*proc_map)(GObj*));
...
sb32 mpObjectProc_ProcFighterDamage(mpCollData* coll_data, GObj* fighter_gobj,
									u32 flags);
...
void mpObjectProc_ProcObjectCollisionDefault(mpCollData* coll_data,
											 GObj* fighter_gobj, u32 flags);
void mpObjectProc_CopyCollDataStats(mpCollData* this_coll_data, Vec3f* pos,
									mpCollData* other_coll_data);
void mpObjectProc_ResetCollDataStats(mpCollData* coll_data);
void ftMap_RunCollisionDefault(GObj* fighter_gobj, Vec3f* pos,
							   mpCollData* coll_data);
void itMap_RunCollisionDefault(GObj* item_gobj, Vec3f* pos,
							   mpCollData* coll_data);
void wpMap_RunCollisionDefault(GObj* weapon_gobj, Vec3f* pos,
							   mpCollData* coll_data);
