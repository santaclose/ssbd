...
sb32 itMap_ProcLRWallCheckGround(mpCollData* coll_data, GObj* item_gobj,
								 u32 flags);
...
sb32 itMap_ProcAllCheckCollEnd(mpCollData* coll_data, GObj* item_gobj,
							   u32 flags);
...
sb32 itMap_ProcAllCheckCollisionFlag(mpCollData* coll_data, GObj* item_gobj,
									 u32 coll_flags);
...
sb32 itMap_CheckCollideAllRebound(GObj* item_gobj, u32 check_flags,
								  f32 mod_vel, Vec3f* pos);

// Set recoil velocity when item hits ground
void itMap_SetGroundCollisionRecoil(Vec3f* vel, Vec3f* ground_angle,
									f32 ground_rebound);

// Check if item should be destroyed upon landing after being dropped / thrown
sb32 itMap_CheckMapCollideThrownLanding(GObj* item_gobj, f32 wall_ceil_rebound,
										f32 ground_rebound,
										void (*proc_status)(GObj*));

// Check if item is landing normally
sb32 itMap_CheckMapCollideLanding(GObj* item_gobj, f32 wall_ceil_rebound,
								  f32 ground_rebound, void (*proc_map)(GObj*));

// Check if item recoils from collision with any type of surface; run proc_map
// if true
sb32 itMap_CheckMapReboundProcAll(GObj* item_gobj, f32 wall_ceil_rebound,
								  f32 ground_rebound, void (*proc_map)(GObj*));
...
sb32 itMap_CheckMapReboundProcGround(GObj* item_gobj, f32 wall_ceil_rebound,
									 void (*proc)(GObj*));
