...
// Macros
#define WEAPON_ALLOC_MAX                                                      \
	32 // Allocate this many weapon user_data structs at once
...
#define WEAPON_FLAG_PROJECT                                                   \
	(1 << 31) // Perform initial collision check when spawning weapon?
...
#define WEAPON_MASK_SPAWN_ALL                                                 \
	0xF // Mask all GObj classes that can spawn weapons?
...
#define WEAPON_REFLECT_TIME_DEFAULT                                           \
	100 // Maximum damage cap for reflected weapons
#define WEAPON_REFLECT_MUL_DEFAULT 1.8F // Universal reflect damage multiplier
#define WEAPON_REFLECT_ADD_DEFAULT                                            \
	0.99F // Added after multiplying weapon's hitbox damage

#define WEAPON_STALE_ADD_DEFAULT                                              \
	0.999F // Bonus 1% added after multiplying hitbox damage with staling
		   // modifier

#define WEAPON_HOP_ANGLE_DEFAULT                                              \
	F_DEG_TO_RAD(                                                             \
		135.0F) // 2.3561945F - Determines whether weapon bounces off a shield
...
struct wpHitbox
{
	gmHitCollisionUpdateState
		update_state; // 0 = disabled, 1 = new hitbox, 2 and 3 =
...
struct wpStruct
{
	...
	sb32 (*proc_hit)(
		GObj*); // Runs when weapon's hitbox collides with a hurtbox
	sb32 (*proc_shield)(
		GObj*); // Runs when weapon's hitbox collides with a shield
	sb32 (*proc_hop)(GObj*); // Runs when weapon bounces off a shield
	sb32 (*proc_setoff)(
		GObj*); // Runs when weapon's hitbox collides with another hitbox
