...
void scTrainingMode_CheckEnterTrainingMenu()
{
	...
	if (gPlayerControllers[player].button_new & HAL_BUTTON_START)
	{
		...
		if (!(fp->is_ignore_startbutton))
		{
			...
			func_ovl2_801157EC();
			ftCommon_ResetControllerInputs(
				gBattleState->player_block[player].fighter_gobj);
			ftCommon_ResetControllerInputs(
				gBattleState->player_block[gTrainingModeStruct.opponent]
					.fighter_gobj);
...
void scTrainingMode_CheckLeaveTrainingMenu()
{
	...
	if (gPlayerControllers[player].button_new
		& (HAL_BUTTON_B | HAL_BUTTON_START))
	{
...
void scTrainingMode_UpdateMenuInputs()
{
	...
	{
		gTrainingModeStruct.button_tap
			= (inputs ^ gTrainingModeStruct.button_hold) & inputs;
...
{
	if (scTrainingMode_CheckUpdateOptionID(&gTrainingModeStruct.cp_menu_option,
										   0, 5)
		!= FALSE)
	{
...
s32 scTrainingMode_GetSpawnableItemCount()
{
	...
	for (item_count = 0; item_gobj != NULL;
...
sb32 scTrainingMode_UpdateItemOption()
{
	...
	if (scTrainingMode_CheckUpdateOptionID(
			&gTrainingModeStruct.item_menu_option, 0, 0x11)
		!= FALSE)
	{
	...
	{
		if ((gPlayerControllers[gSceneData.player_port].button_new
			 & HAL_BUTTON_A)
			&& (gTrainingModeStruct.item_menu_option != 0))
		{
			if (scTrainingMode_GetSpawnableItemCount() <= 3)
			{
				...
				pos = DObjGetStruct(
						  gBattleState->player_block[gSceneData.player_port]
							  .fighter_gobj)
						  ->translate.vec.f;
				...
				itManager_MakeItemSetupCommon(
					NULL, gTrainingModeStruct.item_menu_option + 3, &pos, &vel,
					4);
				func_800269C0(0x9EU);
...
{
	if (scTrainingMode_CheckUpdateOptionID(
			&gTrainingModeStruct.speed_menu_option, 0, 4)
		!= FALSE)
	{
...
sb32 scTrainingMode_UpdateViewOption()
{
	...
	if (scTrainingMode_CheckUpdateOptionID(
			&gTrainingModeStruct.view_menu_option, 0, 2)
		!= FALSE)
	{
		...
		{
			fighter_gobj = gBattleState->player_block[gSceneData.player_port]
							   .fighter_gobj;
			func_ovl2_8010CF44(
				fighter_gobj, 0.0F, 0.0F,
				ftGetStruct(fighter_gobj)->attributes->closeup_cam_zoom, 0.1F,
				28.0F);
...
sb32 scTrainingMode_CheckSpeedFrameFreeze()
{
	if (gTrainingModeStruct.lagframe_wait == 0)
	{
		if (gTrainingModeStruct.frameadvance_wait == 0)
			gTrainingModeStruct.lagframe_wait
				= D_ovl7_8019081C[gTrainingModeStruct.speed_menu_option][0];
		else
		{
	...
	if (gTrainingModeStruct.lagframe_wait == 0)
		gTrainingModeStruct.frameadvance_wait
			= D_ovl7_8019081C[gTrainingModeStruct.speed_menu_option][1];
...
void func_ovl7_8018DA98()
{
		 ...
		 player++)
	{
		if (player == gSceneData.player_port)
		{
			...
			gBattleState->player_block[player].character_kind
				= gSceneData.unk3B;
	...
	gTrainingModeStruct.button_hold = gTrainingModeStruct.button_tap
		= gTrainingModeStruct.button_queue = 0;
	gTrainingModeStruct.rapid_scroll_wait = 30;
...
{
	void* addr = rldm_get_file_with_external_heap(
		(u32)&D_NF_000000FE,
		hal_alloc(rldm_bytes_needed_to_load((u32)&D_NF_000000FE), 0x10));
	gTrainingModeStruct.display_label_sprites
		= (void*)((uintptr_t)addr + (intptr_t)&D_NF_00000000);
	gTrainingModeStruct.display_option_sprites
		= (void*)((uintptr_t)addr + (intptr_t)&D_NF_00000020);
	gTrainingModeStruct.menu_label_sprites
		= (void*)((uintptr_t)addr + (intptr_t)&D_NF_000000BC);
	gTrainingModeStruct.menu_option_sprites
		= (void*)((uintptr_t)addr + (intptr_t)&D_NF_0000013C);
	gTrainingModeStruct.unk_trainmenu_0x34
		= (void*)((uintptr_t)addr + (intptr_t)&D_NF_0000010C);
	gTrainingModeStruct.unk_trainmenu_0x38
		= (void*)((uintptr_t)addr + (intptr_t)&D_NF_000001B8);
}
	...
	gGroundInfo->unk_0x48
		= (void*)(rldm_get_file_external_force(
					  scTrainingMode_Files_BackgroundImageInfo
						  [scTrainingMode_Files_BackgroundImageIDs
							   [gBattleState->gr_kind]]
							  .file_id,
					  (void*)((uintptr_t)gGroundInfo->unk_0x48
							  - (intptr_t)
								  D_ovl7_801907B8[gBattleState->gr_kind]))
				  + scTrainingMode_Files_BackgroundImageInfo
						[scTrainingMode_Files_BackgroundImageIDs[gBattleState
																	 ->gr_kind]]
							.addr);
}
...
// 8018DEDC
SObj* scTrainingMode_MakeStatDisplaySObj(GObj* interface_gobj,
										 scTrainingSprites* tms)
{
...
void scTrainingMode_InitStatDisplayTextInterface()
{
	...
	for (i = 0; i < 4; i++)
	{
		SObj* sobj = scTrainingMode_MakeStatDisplaySObj(
			interface_gobj, &gTrainingModeStruct.display_label_sprites[i]);
...
void scTrainingMode_UpdateDamageDisplay(GObj* interface_gobj, s32 index)
{
	...
	for (i = 0;
		 i < (ARRAY_COUNT(D_ovl7_801907DC) + ARRAY_COUNT(D_ovl7_801907E4)) / 2;
		 i++)
	{
...
{
	s32 damage = gBattleState->player_block[gTrainingModeStruct.opponent]
					 .combo_damage_foe;
	if (damage > 999)
		damage = 999;
	if (damage == 0)
...
void scTrainingMode_MakeDamageDisplayInterface()
{
	...
	omAddGObjRenderProc(interface_gobj, scTrainingMode_UpdateDamageInfo, 0x17U,
						0x80000000, -1);
	omAddGObjCommonProc(interface_gobj, scTrainingMode_UpdateDamageResetWait,
						1, 4);

	for (i = 0; i < 3; i++)
	{
		sobj = func_ovl0_800CCFDC(
			interface_gobj, gTrainingModeStruct.display_option_sprites[0]);
	...
	sobj = func_ovl0_800CCFDC(interface_gobj,
							  gTrainingModeStruct.display_option_sprites[38]);
	scTrainingMode_InitSObjColors(sobj);
...
void scTrainingMode_UpdateComboDisplay(GObj* interface_gobj, s32 index)
{
	...
	for (i = 0;
		 i < (ARRAY_COUNT(D_ovl7_801907E8) + ARRAY_COUNT(D_ovl7_801907EC)) / 2;
		 i++)
	{
...
{
	s32 combo = gBattleState->player_block[gTrainingModeStruct.opponent]
					.combo_count_foe;
...
void scTrainingMode_MakeComboDisplayInterface()
{
	...
	omAddGObjRenderProc(interface_gobj, scTrainingMode_UpdateComboInfo, 0x17U,
						0x80000000, -1);
	omAddGObjCommonProc(interface_gobj, scTrainingMode_UpdateComboResetWait, 1,
						4);

	for (i = 0; i < 2; i++)
	{
		SObj* sobj = func_ovl0_800CCFDC(
			interface_gobj, *gTrainingModeStruct.display_option_sprites);
		scTrainingMode_InitSObjColors(sobj);
		sobj->pos.y = 36.0F;
	}
...
void scTrainingMode_InitSpeedDisplaySprite()
{
	...
	sobj->sprite
		= *gTrainingModeStruct
			   .display_option_sprites[gTrainingModeStruct.speed_menu_option
									   + 27];
}
...
void scTrainingMode_MakeSpeedDisplayInterface()
{
	...
	gTrainingModeStruct.speed_display_gobj = interface_gobj
		= omMakeGObjCommon(omGObj_Kind_Interface, NULL, 0xB, 0x80000000);
	omAddGObjRenderProc(interface_gobj, func_ovl0_800CCF00, 0x17, 0x80000000U,
						-1);
	sobj = func_ovl0_800CCFDC(
		interface_gobj,
		gTrainingModeStruct
...
void scTrainingMode_InitCPDisplaySprite()
{
	...
	sobj->sprite
		= *gTrainingModeStruct
			   .display_option_sprites[gTrainingModeStruct.cp_menu_option + 31];
}
...
void scTrainingMode_InitItemDisplaySprite()
{
	...
	root_sobj->sprite
		= *gTrainingModeStruct
			   .display_option_sprites[gTrainingModeStruct.item_hold + 10];
	root_sobj->pos.x = (f32)(292 - root_sobj->sprite.width);
	next_sobj->pos.x
		= (f32)(292 - root_sobj->sprite.width) - next_sobj->sprite.width;
}

// 8018E9AC
void scTrainingMode_UpdateItemDisplay(s32 interface_gobj)
{
	ftStruct* fp = ftGetStruct(
		gBattleState->player_block[gSceneData.player_port].fighter_gobj);
	...
	if (item_gobj != NULL)
	{
		...
		item_id = (ip->it_kind <= It_Kind_CommonEnd)
					  ? scGetTrainingModeItemKind(ip->it_kind)
					  : 0;
	}
	else
		item_id = 0;
...
void scTrainingMode_MakeItemDisplayInterface()
{
	...
	gTrainingModeStruct.item_display_gobj = interface_gobj
		= omMakeGObjCommon(omGObj_Kind_Interface, NULL, 0xB, 0x80000000);
	omAddGObjRenderProc(interface_gobj, scTrainingMode_UpdateItemDisplay, 0x17,
						0x80000000, -1);
	sobj = func_ovl0_800CCFDC(interface_gobj,
							  gTrainingModeStruct.display_option_sprites[37]);
	...
	sobj = func_ovl0_800CCFDC(interface_gobj,
							  gTrainingModeStruct.display_option_sprites[0]);
	sobj->pos.y = 36.0F;
	scTrainingMode_InitSObjColors(sobj);

	sobj = func_ovl0_800CCFDC(interface_gobj,
							  gTrainingModeStruct.display_option_sprites[36]);
	sobj->pos.y = 36.0F;
	scTrainingMode_InitSObjColors(sobj);
}
...
void scTrainingMode_MakeMenuLabelsInterface()
{
	...
	gTrainingModeStruct.menu_label_gobj = interface_gobj
		= omMakeGObjCommon(0x3F8U, NULL, 0xE, 0x80000000);
	omAddGObjRenderProc(interface_gobj, func_ovl0_800CCF00, 0x17, 0x80000000,
						-1);
	...
	{
		SObj* sobj = scTrainingMode_MakeStatDisplaySObj(
			interface_gobj, &gTrainingModeStruct.menu_label_sprites[i]);
...
void scTrainingMode_InitMenuOptionSpriteAttrs()
{
	s32 i;
	for (i = 0; i < 31; i++)
		gTrainingModeStruct.menu_option_sprites[i]->attr
...
void scTrainingMode_MakeMainMenuInterface()
{
	omAddGObjRenderProc(
		...
		scTrainingMode_RenderMainMenu, 0x16, 0x80000000, -1);
}
...
{
	s32 i;
	for (i = scTrainingMenu_OptionSprite_CPStart;
		 i <= scTrainingMenu_OptionSprite_CPEnd; i++)
	{
...
void scTrainingMode_InitCPOptionSprite()
{
	...
	sobj->sprite
		= *gTrainingModeStruct
			   .menu_option_sprites[gTrainingModeStruct.cp_menu_option
...
void scTrainingMode_InitItemOptionSprite()
{
	...
	sobj->sprite
		= *gTrainingModeStruct
			   .menu_option_sprites[gTrainingModeStruct.item_menu_option
									+ scTrainingMenu_OptionSprite_ItemStart];

	sobj->pos.x = 191 - (sobj->sprite.width / 2);
	sobj->pos.y = (gTrainingModeStruct.item_menu_option
...
{
	s32 i;
	for (i = scTrainingMenu_OptionSprite_ItemStart;
		 i <= scTrainingMenu_OptionSprite_ItemEnd; i++)
	{
		Sprite* sprite = gTrainingModeStruct.menu_option_sprites[i];
		sprite->red = 0xFF;
		sprite->green = 0xFF;
...
{
	s32 i;
	for (i = scTrainingMenu_OptionSprite_SpeedStart;
		 i <= scTrainingMenu_OptionSprite_SpeedEnd; i++)
	{
		Sprite* sprite = gTrainingModeStruct.menu_option_sprites[i];
		sprite->red = 0xFF;
		sprite->green = 0xFF;
...
void scTrainingMode_InitSpeedOptionSprite()
{
	...
	sobj->sprite
		= *gTrainingModeStruct
			   .menu_option_sprites[gTrainingModeStruct.speed_menu_option
...
void scTrainingMode_InitViewOptionSprite()
{
	...
	sobj->sprite
		= *gTrainingModeStruct
			   .menu_option_sprites[gTrainingModeStruct.view_menu_option
...
void scTrainingMode_MakeViewOptionInterface()
{
	...
	omAddGObjCommonProc(interface_gobj, scTrainingMode_UpdateMagnifyWait, 1,
						4);
}
...
{
	gTrainingModeStruct.hscroll_option_sobj[0]
		= SObjGetStruct(gTrainingModeStruct.cp_option_gobj);
	gTrainingModeStruct.hscroll_option_sobj[1]
		= SObjGetStruct(gTrainingModeStruct.item_option_gobj);
	gTrainingModeStruct.hscroll_option_sobj[2]
		= SObjGetStruct(gTrainingModeStruct.speed_option_gobj);
	gTrainingModeStruct.hscroll_option_sobj[3]
		= SObjGetStruct(gTrainingModeStruct.view_option_gobj);
}
...
{
	SObj* root_sobj
		= SObjGetStruct(gTrainingModeStruct.arrow_option_gobj); // Left arrow
	...
	if (gTrainingModeStruct.main_menu_option <= scTrainingMenu_Main_ScrollEnd)
	{
		SObj* option_sobj
			= gTrainingModeStruct
				  .hscroll_option_sobj[gTrainingModeStruct.main_menu_option];

		root_sobj->pos.x = 137.0F;
		next_sobj->pos.x = 237.0F;

		if ((gTrainingModeStruct.main_menu_option == scTrainingMenu_Main_Item)
			&& (gTrainingModeStruct.item_menu_option
				== scTrainingMenu_Item_MotionSensorBomb))
			root_sobj->pos.y = next_sobj->pos.y
				= (s32)(option_sobj->pos.y + 5.0F);
		else
			root_sobj->pos.y = next_sobj->pos.y
				= (s32)(option_sobj->pos.y + 3.0F);
...
void scTrainingMode_MakeOptionArrowInterface()
{
	...
	scTrainingMode_InitOptionArrowColors(func_ovl0_800CCFDC(
		interface_gobj,
		gTrainingModeStruct
			.menu_option_sprites[scTrainingMenu_OptionSprite_LeftArrow]));
	scTrainingMode_InitOptionArrowColors(func_ovl0_800CCFDC(
		interface_gobj,
		gTrainingModeStruct
			.menu_option_sprites[scTrainingMenu_OptionSprite_RightArrow]));
...
void func_ovl7_8018F804() // Unused?
{
	s32 i;
	for (i = 0; i < 6; i++)
		gTrainingModeStruct.unk_trainmenu_0x34[i].sprite->attr
			= SP_TEXSHUF | SP_TRANSPARENT;
}
...
void func_ovl7_8018F874() // Unused?
{
	...
	sobj->sprite
		= *gTrainingModeStruct
			   .unk_trainmenu_0x34[gTrainingModeStruct.main_menu_option]
			   .sprite;
	sobj->pos.x = gTrainingModeStruct
					  .unk_trainmenu_0x34[gTrainingModeStruct.main_menu_option]
					  .pos.x;
}
...
void func_ovl7_8018F984() // Unused?
{
	s32 i;
	for (i = 0; i < 28; i++)
		gTrainingModeStruct.unk_trainmenu_0x38[i].sprite->attr
			= SP_TEXSHUF | SP_TRANSPARENT;
}
...
s32 scTrainingMode_GetMenuOptionSpriteID()
{
	...
	case scTrainingMenu_Main_CP:
		return gTrainingModeStruct.cp_menu_option
			   + scTrainingMenu_OptionSprite_CPStart;

	case scTrainingMenu_Main_Item:
		return gTrainingModeStruct.item_menu_option
			   + scTrainingMenu_OptionSprite_ItemStart;

	case scTrainingMenu_Main_Speed:
		return gTrainingModeStruct.speed_menu_option
			   + scTrainingMenu_OptionSprite_SpeedStart;

	case scTrainingMenu_Main_View:
		return gTrainingModeStruct.view_menu_option
			   + scTrainingMenu_OptionSprite_ViewStart;
...
void func_ovl7_8018FA54() // Unused but referenced?
{
	...
	{
		sobj->sprite
			= *gTrainingModeStruct.unk_trainmenu_0x38[sprite_id].sprite;
		sobj->pos.x = gTrainingModeStruct.unk_trainmenu_0x38[sprite_id].pos.x;
		sobj->pos.y
			= (sprite_id == scTrainingMenu_OptionSprite_ItemMotionSensorBomb)
				  ? 178.0F
				  : 182.0F;
		sobj->sprite.attr &= ~SP_HIDDEN;
	}
...
void func_ovl7_8018FB40() // Unused?
{
	...
	gTrainingModeStruct.combo0 = interface_gobj
		= omMakeGObjCommon(omGObj_Kind_Interface, NULL, 0xE, 0x80000000);
	omAddGObjRenderProc(interface_gobj, func_ovl0_800CCF00, 0x17, 0x80000000,
						-1);
...
void scTrainingMode_UpdateCursorPosition()
{
	...
	SObj* text_sobj
		= gTrainingModeStruct
			  .vscroll_option_sobj[gTrainingModeStruct.main_menu_option][0];
...
void scTrainingMode_MakeMenuCursorInterface()
{
	...
	gTrainingModeStruct.cursor_gobj = interface_gobj
		= omMakeGObjCommon(omGObj_Kind_Interface, NULL, 0xE, 0x80000000);
	omAddGObjRenderProc(interface_gobj, func_ovl0_800CCF00, 0x17, 0x80000000,
						-1);
	target_sprite = func_ovl0_800CCFDC(
		interface_gobj,
		gTrainingModeStruct
			.menu_option_sprites[scTrainingMenu_OptionSprite_Cursor]);
...
{
	SObj* arrow_sobj
		= SObjGetStruct(gTrainingModeStruct.arrow_option_gobj)->next;

	gTrainingModeStruct.vscroll_option_sobj[0][0]
		= SObjGetStruct(gTrainingModeStruct.menu_label_gobj);
	gTrainingModeStruct.vscroll_option_sobj[0][1] = arrow_sobj;

	gTrainingModeStruct.vscroll_option_sobj[1][0]
		= gTrainingModeStruct.vscroll_option_sobj[0][0]->next;
	gTrainingModeStruct.vscroll_option_sobj[1][1] = arrow_sobj;

	gTrainingModeStruct.vscroll_option_sobj[2][0]
		= gTrainingModeStruct.vscroll_option_sobj[1][0]->next;
	gTrainingModeStruct.vscroll_option_sobj[2][1] = arrow_sobj;

	gTrainingModeStruct.vscroll_option_sobj[3][0]
		= gTrainingModeStruct.vscroll_option_sobj[2][0]->next;
	gTrainingModeStruct.vscroll_option_sobj[3][1] = arrow_sobj;

	gTrainingModeStruct.vscroll_option_sobj[4][0]
		= gTrainingModeStruct.vscroll_option_sobj[4][1]
		= gTrainingModeStruct.vscroll_option_sobj[3][0]->next;
	gTrainingModeStruct.vscroll_option_sobj[5][0]
		= gTrainingModeStruct.vscroll_option_sobj[5][1]
...
void scTrainingMode_RenderCursorUnderline(GObj* interface_gobj)
{
	...
	gDPSetFillColor(
		gDisplayListHead[0]++,
		rgba32_to_fill_color(GPACK_RGBA8888(0xFF, 0x00, 0x00, 0xFF),
							 gDisplayListHead));
	gDPFillRectangle(gDisplayListHead[0]++, gTrainingModeStruct.cursor_ulx,
					 gTrainingModeStruct.cursor_uly,
					 gTrainingModeStruct.cursor_lrx,
					 gTrainingModeStruct.cursor_lry);
	gDPPipeSync(gDisplayListHead[0]++);
	gDPSetCycleType(gDisplayListHead[0]++, G_CYC_1CYCLE);
	gDPSetRenderMode(gDisplayListHead[0]++, G_RM_AA_ZB_OPA_SURF,
...
void scTrainingMode_UpdateCursorUnderline()
{
	...
	SObj* arrow_sobj
		= gTrainingModeStruct
			  .vscroll_option_sobj[gTrainingModeStruct.main_menu_option][1];
	s32 offset;

	gTrainingModeStruct.cursor_ulx = text_sobj->pos.x - 13.0F;

	offset
		= ((gTrainingModeStruct.main_menu_option == scTrainingMenu_Main_Reset)
		   || (gTrainingModeStruct.main_menu_option
			   == scTrainingMenu_Main_Exit))
			  ? 2
			  : -2;

	gTrainingModeStruct.cursor_lrx
		= offset + (arrow_sobj->pos.x + arrow_sobj->sprite.width);
	gTrainingModeStruct.cursor_uly
		= text_sobj->pos.y + text_sobj->sprite.height + (-1.0F);
...
{
	ftStruct* fp = ftGetStruct(
		gBattleState->player_block[gTrainingModeStruct.opponent].fighter_gobj);

	if (fp->status_info.pl_kind == Pl_Kind_Com)
	{
		fp->fighter_com.behavior_set
			= scTrainingMode_CPOpponent_BehaviorKind[gTrainingModeStruct
														 .cp_menu_option];
...
void scTrainingMode_InitTrainingMode()
{
		 ...
		 player++)
	{
		...
		ftManager_SetFileDataKind(
			gBattleState->player_block[player].character_kind);
		player_spawn.ft_kind
			= gBattleState->player_block[player].character_kind;
		mpCollision_GetPlayerMPointPosition(player, &player_spawn.pos);
		player_spawn.lr_spawn
			= (player_spawn.pos.x >= 0.0F) ? LR_Left : LR_Right;
		player_spawn.team = gBattleState->player_block[player].team_index;
		player_spawn.player = player;
		player_spawn.model_lod
			= ((gBattleState->pl_count + gBattleState->cp_count) < 3)
				  ? ftParts_LOD_HighPoly
				  : ftParts_LOD_LowPoly;
		player_spawn.costume
			= gBattleState->player_block[player].costume_index;
		...
		player_spawn.anim_heap = ftManager_AllocAnimHeapKind(
			gBattleState->player_block[player].character_kind);
...
void scTrainingMode_SetGeometryRenderLights(Gfx** display_list)
{
	...
	ftRender_Lights_DisplayLightReflect(display_list, gMapLightAngleX,
										gMapLightAngleY);
}
...
void scManager_TrainingMode_InitScene()
{
	...
	D_ovl7_8019088C.arena_size
		= ((uintptr_t)&lOverlay7ArenaHi - (uintptr_t)&lOverlay7ArenaLo);
...
void func_ovl7_801906D0()
{
	...
	rldm_load_files_into(
		D_ovl2_80116BD0, ARRAY_COUNT(D_ovl2_80116BD0), gCommonSpriteFiles,
		hal_alloc(rldm_bytes_need_to_load(D_ovl2_80116BD0,
