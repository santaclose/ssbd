...
{
	ftScriptInfo script_info[ftMotion_Common_SpecialStart]; // Array size = last
															// animation ID?
};
...
struct ftHitCollisionLog // Might have to return once structs are cleaned up
						 // (alas once forward declarations are implemented to
						 // replace void* with struct*)
{
...
struct ftAttributes
{
	...
	f32 jostle_width; // ???
	f32 jostle_x;
	sb32 is_metallic; // So far only seen this used to determine whether the
					  // character makes blue sparks or gray metal dust
					  // particles when hit; used by Metal Mario and Samus
	f32 cam_offset_y;
	...
	Vec3f hit_detect_range; // This is a radius around the fighter within which
							// hitbox detection can occur
	s32 unk_ftca_0x29C;
	ftPartsUnkIndexTable* unk_ftca_0x2A0;
	s32 gfx_joint_cycle_index[5]; // The game will cycle through these joints
								  // when applying certain particles such as
	...
	DObjDesc* dobj_lookup; // WARNING: Not actually DObjDesc* but I don't know
						   // what this struct is or what its bounds are; bunch
						   // of consecutive floats
	s32* unk_joint[8];
	...
	f32 unk_0x31C;
	f32 unk_0x320;
	Vec3f* unk_0x324; // Pointer to some array of vectors, something to do with
					  // joints
...
struct ftStruct
{
	...
	u8 costume;
	u8 shade; // i.e. When multiple instances of the same character costume are
			  // in-game
	u8 handicap;
	u8 cp_level;
	s8 stock_count;
	u8 team_order; // Order number if this fighter is a "VS *character* Team"
				   // member; used to check for bonuses such as Yoshi Rainbow
	u8 unk_0x16;
	s32 player_number; // Player's number? (Note: NOT player port, e.g. if
					   // players 2 and 4 are in a match,
	// player 2 will be number 1 and player 4 will be number 2; used to match
	// fighters and items?)
	...
	s32 percent_damage;
	s32 damage_resist; // Resits a specific amount of % damage before breaking,
					   // effectively damage-based armor
	s32 shield_health;
	f32 unk_ft_0x38;
	s32 x3C_unk;
	...
	ub32 is_playertag_movie : 1; // Also skips rendering player indicator? Used
								 // only in "Master Hand defeated" cinematic
								 // from what I can tell so far
	ub32 is_playing_gfx : 1;
	u32 joint_cycle_array_index : 4; // Goes up to 5 by default; index of the
									 // array from gfx_joint_cycle_index from
	...
	ub32 is_disable_control : 1; // Fighter cannot be controlled if TRUE;
								 // enabled when training mode menu is up
	ub32 is_hitstun : 1;
	...
	u32 camera_mode : 4;
	ub32 is_special_interrupt : 1; // Whether move can be interrupted by Link's
								   // boomerang? Have not seen this used
								   // anywhere else
	ub32 x192_flag_b1 : 1;
	...
	s32 breakout_wait; // Frames until fighter breaks out of shield break /
					   // sleep / Cargo Throw
	s8 breakout_lr;	   // Whether victim is mashing left or right
	s8 breakout_ud;	   // Whether victim is mashing up or down

	u8 shuffle_frame_index; // Ranges from 0-3; position of fighter's model
							// vibration is adjusted based on this index when
							// receiving hitlag
	u8 shuffle_index_max;	// How many iterations the frame index increments
							// before looping back to 0;
	u8 is_shuffle_electric; // Fighter vibrates horizontally instead of
							// vertically if hit by an electric attack
	...
	s32 attack_id;
	u16 motion_count; // This is used to tell the game not to stale multihit
					  // attacks
	...
	u16 attack_hit_count; // Number of times this fighter successfully dealt
						  // damage
	s32 shield_attack_damage;
	f32 attack_rebound; // Actually 2x staled damage?
	s32 lr_attack;
	s32 shield_damage;
	s32 shield_damage_total; // shield_damage + hitbox damage + hitbox shield
							 // damage, does not persist?
	s32 lr_shield;
	...
	f32 knockback_resist_status;  // Resist this many units of knockback,
								  // effectively knockback-based armor
	...
	GfxColorAlpha fog_color;   // Used only by Master Hand, when in the
