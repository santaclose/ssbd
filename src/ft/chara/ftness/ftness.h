...
#define FTNESS_PKTHUNDER_COLLIDE_X                                            \
	250.0F // Width of Ness + PK Thunder collision detection box
#define FTNESS_PKTHUNDER_COLLIDE_Y                                            \
	370.0F // Height of Ness + PK Thunder collision detection box
#define FTNESS_PKTHUNDER_END_DELAY 30
#define FTNESS_PKTHUNDER_GRAVITY_DELAY 25
#define FTNESS_PKTHUNDER_LANDING_LAG                                          \
	F_PCT_TO_DEC(17.0F) // Divides landing animation length by this value
#define FTNESS_PKTHUNDER_FALLSPECIAL_DRIFT 0.6F
#define FTNESS_PKJIBAKU_DELAY 30
#define FTNESS_PKJIBAKU_ANIM_LENGTH 28
#define FTNESS_PKJIBAKU_HALT_ANGLE                                            \
	F_DEG_TO_RAD(155.0F) // Maximum angle of stage collision that doesn't
						 // cancel PK Thunder Blast? (2.7052603F)
#define FTNESS_PKJIBAKU_VEL 200.0F
#define FTNESS_PKJIBAKU_DECELERATE 6.142857F
#define FTNESS_PKJIBAKU_PASS_FRAME_END                                        \
	((float)FTNESS_PKJIBAKU_ANIM_LENGTH - 3.0F)
...
{
	s32 pkjibaku_delay; // Frames before PK Thunder Blast can be initiated (PK
						// Thunder will go through Ness while this timer is
						// greater than 0)
	s32 pkthunder_end_delay; // Frames before PK Thunder transitions to its
							 // ending animation after the projectile despawns
	s32 pkthunder_gravity_delay; // Frames before Ness is affected by gravity
								 // while performing PK Thunder
	GObj* pkthunder_gobj;		 // Pointer to the PK Thunder object
	s32 pkjibaku_anim_length;	 // Number of frames PK Thunder Blast will loop
	f32 pkjibaku_angle;	 // Angle of PK Thunder Blast (converted to 2D velocity
						 // vector with sin(vel) and cos(vel))
	Vec3f pkthunder_pos; // Might be PK Thunder's collision detection when
						 // checking if Ness is hitting himself with it?

} ftNess_SpecialHi_StatusVars;

typedef struct ftNess_SpecialLw_StatusVars
{
	s32 release_lag; // Frames PSI Magnet automatically stays in the hold
					 // animation after initiating the move
	sb32 is_release; // Bool to check if PSI Magnet should be dropped (alas the
					 // B button is no longer being held)
	s32 gravity_delay; // Frames before Ness is affected by gravity after
					   // initiating PSI Magnet
